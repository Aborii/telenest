...
- `TelegramUserService` — act **as your own account**: `getMe`, `getDialogs`, `getMessages`, `sendMessage`/`sendToSelf`, plus media (`sendFile`, `downloadMedia`, `downloadProfilePhoto`), chat/channel management (`joinChannel`, `leaveChannel`, `getParticipants`, `searchMessages`, `getFullChat`), and message operations (`editMessage`, `deleteMessages`, `forwardMessages`, `markAsRead`, `pinMessage`).
