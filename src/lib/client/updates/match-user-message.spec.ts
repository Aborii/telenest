...
describe('matchesUserMessageFilter', () => {
  ...
  describe('direction', () => {
    it('incoming matches only non-self messages', () => {
      expect(
        matchesUserMessageFilter(msg({ out: false }), { incoming: true }),
      ).toBe(true);
      expect(
        matchesUserMessageFilter(msg({ out: true }), { incoming: true }),
      ).toBe(false);
    });

    it('outgoing matches only self messages', () => {
      expect(
        matchesUserMessageFilter(msg({ out: true }), { outgoing: true }),
      ).toBe(true);
      expect(
        matchesUserMessageFilter(msg({ out: false }), { outgoing: true }),
      ).toBe(false);
    });
  });
  ...
  describe('pattern', () => {
    it('tests a RegExp against the text', () => {
      expect(
        matchesUserMessageFilter(msg({ text: 'ping' }), { pattern: /^ping$/ }),
      ).toBe(true);
      expect(
        matchesUserMessageFilter(msg({ text: 'pong' }), { pattern: /^ping$/ }),
      ).toBe(false);
    });

    it('requires exact equality for a string', () => {
      expect(
        matchesUserMessageFilter(msg({ text: 'hi' }), { pattern: 'hi' }),
      ).toBe(true);
      expect(
        matchesUserMessageFilter(msg({ text: 'hi there' }), { pattern: 'hi' }),
      ).toBe(false);
    });
    ...
    it('is stable across calls for a globally-flagged RegExp (no lastIndex drift)', () => {
      ...
      expect(matchesUserMessageFilter(msg({ text: 'ping' }), filter)).toBe(
        true,
      );
      expect(matchesUserMessageFilter(msg({ text: 'ping' }), filter)).toBe(
        true,
  ...
  describe('chatId', () => {
    it('matches a single id', () => {
      expect(
        matchesUserMessageFilter(msg({ peerId: '555' }), { chatId: 555 }),
      ).toBe(true);
      expect(
        matchesUserMessageFilter(msg({ peerId: '777' }), { chatId: 555 }),
      ).toBe(false);
    });

    it('matches any id in an array', () => {
      expect(
        matchesUserMessageFilter(msg({ peerId: '777' }), {
          chatId: ['555', '777'],
        }),
      ).toBe(true);
      expect(
        matchesUserMessageFilter(msg({ peerId: '999' }), {
          chatId: ['555', '777'],
        }),
      ).toBe(false);
    });
  ...
  it('ANDs every present criterion', () => {
    ...
    expect(
      matchesUserMessageFilter(message, {
        incoming: true,
        pattern: /ping/,
        chatId: '555',
      }),
